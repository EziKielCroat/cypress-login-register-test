...
function Register () {
    ...
    return(
        <>
            ...
            <EmailInput style={{top: "15%"}} placeholder="Type your name here" value={name} onChange={(e) => setName(e.target.value)} type="email "></EmailInput>
            ...
            <LogInWithEmailButton onClick={() => {registerWithEmailAndPassword(name, email, password)}}>Register with email</LogInWithEmailButton>
            <Button  onClick={() => {signInWithGoogle()}}>Register with google</Button>
            <DontHaveanAccount onClick={()=>{navigate("/login")}}>Already have an account? Log in!</DontHaveanAccount>
            <ToastContainer />
        </>
    )
}
